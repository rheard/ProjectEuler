...
_deal_set = []
with open('ProjectEuler/poker.txt', 'r') as rb:
    _deal_set = [[sorted(x[:5], reverse=True), sorted(x[5:], reverse=True)] for x in ([(_values.index(x[0]), x[1]) for x in x.strip().split(' ')] for x in rb.readlines())]
'''
The dealset should be in the format:
...
def winner(deal):
    deal = list({'index': i, 'hand': sorted(x, reverse=True)} for i, x in enumerate(deal))

    for player in deal:
        player['flush'] = [card[0] for card in player['hand']] if len(set(card[1] for card in player['hand'])) == 1 else []
        player['straight'] = player['hand'][0][0] if all(player['hand'][0][0] - i == player['hand'][i][0] for i in range(1, 5)) else -1
    ...
    for player in deal:
        ...
        player['full_house'] = (-1, -1) if player['3kind'] == -1 or len(player['2kind']) == 0 else (player['3kind'], player['2kind'][0])
    ...
    if any(len(player['flush']) != 0 for player in deal):
        ...
        for i in range(5):
            max_flush_val = max(player[i] for player in deal)
...
def solve(deal_set=_deal_set):
    win_count = [0, 0]
    for deal in _deal_set:
...
if __name__ == '__main__':
    answer = solve()
    print(answer)
    with open('p054_ans.txt', 'w') as wb:
        wb.write(str(answer))
